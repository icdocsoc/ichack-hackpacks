...
- Ports: to bind a port from the image to a port on the host, use the `-p` flag: `-p 8080:80` will forward *container port 80* to *host port 8080*, i.e. navigating to `localhost:8080` in a web browser will show whatever is on the container's port `80`.
